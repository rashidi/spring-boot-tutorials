...
We include `spring-ai-starter-vector-store-redis` alongside Testcontainers in `build.gradle.kts`:
...
dependencies {
    ...
    testImplementation("com.redis.testcontainers:testcontainers-redis:2.2.3")
...
spring.ai.vectorstore.redis.index=document_index
spring.ai.vectorstore.redis.prefix=document:
----

Setting `initialize-schema=true` prompts Spring AI to create the requisite index for storing the vectors.
...
public class TestcontainersConfiguration {
    ...
    RedisContainer redisContainer() {
        return new RedisContainer(RedisContainer.DEFAULT_IMAGE_NAME.withTag("7.2.4-v2"));
    }
