...
link:{source-main}/DocumentItem.java[`DocumentItem`] serves as the domain record for ingesting content and metadata:
...
link:{source-main}/SemanticSearchService.java[`SemanticSearchService`] wraps Spring AI's `VectorStore` to ingest documents and execute similarity searches:
...
To ensure tests execute hermetically without external API dependencies or rate limits, link:{source-test}/SemanticSearchIntegrationTests.java[`SemanticSearchIntegrationTests`] defines an in-memory `EmbeddingModel`.
