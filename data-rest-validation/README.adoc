...
There are two `@Entity` classes, link:src/main/java/zin/rashidi/boot/data/rest/book/Author.java[Author] and link:src/main/java/zin/rashidi/boot/data/rest/book/Book.java[Book]. Both classes are accompanied by `JpaRepository` classes - link:src/main/java/zin/rashidi/boot/data/rest/book/AuthorRepository.java[AuthorRepository] and link:src/main/java/zin/rashidi/boot/data/rest/book/BookRepository.java[BookRepository].
...
We will implement a validation that ensures that `Author` in `Book` is not `INACTIVE`. To do this, we will create a custom validator class, link:src/main/java/zin/rashidi/boot/data/rest/book/BeforeCreateBookValidator.java[BeforeCreateBookValidator].
...
As we can see, the validator class implements `Validator` interface and overrides `supports` and `validate` methods. The `supports` method checks if the class is `Book` and the `validate` method checks if the `Author` is `INACTIVE`. Next we will inform Spring about our `Validator` through link:src/main/java/zin/rashidi/boot/data/rest/book/BookValidatorConfiguration.java[BookValidatorConfiguration].
...
include::src/test/java/zin/rashidi/boot/data/rest/book/CreateBookTests.java[lines=89]
----

Full implementation of the test can be found in link:src/test/java/zin/rashidi/boot/data/rest/book/CreateBookTests.java[CreateBookTests].
